...
public class XMLConfigBuilder {
    ...
    public static Configuration loadConfiguration(InputStream in){
        try{
            ...
            //4.使用xpath 中指定节点的方式，获取所有property节点
            System.out.println("...///");
            List<Node> propertyElements = root.selectNodes("//property");
            System.out.println("...///");
            //5.遍历节点
            for(Node property : propertyElements){
            ...
            //遍历集合
            for (Node property : mapperElements) {
                if(property.getNodeType() == Node.ELEMENT_NODE) {
                    Element mapperElement = (Element) property;
     ...
     * @return map中包含了获取的唯一标识（key 是由Mapper接口的全限定类名和方法名组成）
    ...
    private static Map<String,SqlMapper> loadMapperConfiguration(String mapperPath) throws IOException{
        ...
        try {
            ...
            //6.遍历select节点集合
            for (Node property : selectElements) {
                if (property.getNodeType() == Node.ELEMENT_NODE) {
                    Element selectElement = (Element) property;
